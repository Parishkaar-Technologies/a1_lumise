      ...
      if (handleName === 'a1_handle_r') {
        ...
        switch (groupName) {
          ...
          case 'a1_dotgrid': {
            ...
            const newCols = Math.max(1, (a1_drag_origin?.initialCols || 5) + deltaCols);
          ...
          // case 'a1_box': {
          //   const dx = pointer.x - a1_drag_origin.pointerX;
      ...
      if ((handleName === 'a1_handle_t' || handleName === 'a1_handle_b') && a1_vertical_drag_origin) {
        ...
        switch (groupName) {
          case 'a1_lineset': {
            ...
            // const deltaY = pointer.y - a1_vertical_drag_origin.y;
            // let change = Math.round(deltaY / spacing);
            // if (direction === 'top') change = -change;
          ...
          case 'a1_dotgrid': {
            const spacingY = group.metadata?.spacingY || 20;
            const rowsInput = document.getElementById('a1_edit_rows');
            const initialRows = a1_vertical_drag_origin.initialRows;

            let delta;
            if (direction === 'top') {
              delta = a1_vertical_drag_origin.initialTopEdge - pointer.y;
            } else {
              delta = pointer.y - a1_vertical_drag_origin.initialBottomEdge;
            }

            const deltaRows = Math.round(delta / spacingY);
            const newRows = Math.max(1, initialRows + deltaRows);

            if (rowsInput && parseInt(rowsInput.value) !== newRows) {
              rowsInput.value = newRows;
              rowsInput.dispatchEvent(new Event('input', { bubbles: false }));
            }
            break;
          }
