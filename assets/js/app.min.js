...
jQuery(document).ready(function ($) {
  ...
  let _a1_dot_color = "#000000"; // default black
  let _a1_dimension_text_object = null; // This will hold our persistent fabric.Text object
  ...
  window.lumise = {
    ...
    objects: {
      ...
      lumise: {
        ...
        "a1_dotgrid": function (ops, callback) {
          const dots = [];
          ...
          const group = new fabric.Group(dots, {
            ...
            opacity: ops.opacity !== undefined ? ops.opacity : 1,
            visible: ops.visible !== false,
            metadata: ops.metadata || {}, // This could include rows, cols, spacing, etc.
          });

          console.log('✅ Reconstructed a1_dotgrid group with', dots.length, 'dots:', group);

          callback(group);
        },

        "a1_box": function (ops, callback) {
          const meta = ops.metadata || {};
          const width = meta.width || 200;
          const height = meta.height || 100;
          const thickness = meta.thickness || 2;
          const color = meta.color || '#000000';
          ...
          const rect = new fabric.Rect({
            ...
            visible: ops.visible !== false,
            name: ops.name || 'Box',
            id: ops.id || 'a1_box_' + Date.now(),
            object_type: 'a1_box',
            object_id: ops.object_id || 'a1_box_' + Date.now(),
            metadata: {
              width,
              height,
              thickness,
          ...
          console.log('✅ Reconstructed a1_box as single stroked rectangle', width, 'x', height);
          callback(rect);
        },

        "a1_checkboxes": function (ops, callback) {
          const checkboxes = [];

          if (Array.isArray(ops.objects)) {
            ops.objects.forEach(obj => {
              const checkbox = new fabric.Rect({
                ...obj, // preserve original props
                left: obj.left,
                top: obj.top,
                width: obj.width || 20,
          ...
          const group = new fabric.Group(checkboxes, {
            left: ops.left,
            top: ops.top,
            originX: ops.originX || 'center',
            originY: ops.originY || 'center',
            type: 'a1_checkboxes',
            fill: 'transparent',
            name: ops.name || 'CheckBoxes Group',
            angle: ops.angle || 0,
            scaleX: ops.scaleX || 1,
            scaleY: ops.scaleY || 1,
            selectable: true,
            ...
            visible: ops.visible !== false,
            metadata: ops.metadata || {},
            // ⚠️ Do not add fill/stroke here
          });

          console.log('✅ Reconstructed a1_checkboxes group with', checkboxes.length, 'checkboxes:', group);
        ...
        "text-fx": function (ops, callback) {
          const isEmpty = (val) =>
            typeof val === "undefined" || val === null || val === "" || val === "undefined" || val === "null";

          // 👇 NEW: fallback if text missing
          if (isEmpty(ops.text)) {
            console.warn("⚠️ No text provided in ops.text — using fallback 'Sample'");
            ops.text = "Sample"; // or "Untitled FX"
          }

          const safeText = typeof ops.text === 'string' ? ops.text.trim() : '';
          if (safeText === "") {
            console.warn("❌ Empty text string passed to text-fx");
            return;
          }


          const baseText = new fabric.Text(safeText, ops);
          if (baseText.width === 0 || baseText.height === 0) {
            console.warn("⏳ Waiting for font to render (text-fx base)...");
            return setTimeout(() => lumise.objects["text-fx"](ops, callback), 100);
          }

          ops.width = baseText.width;
          ops.height = baseText.height;
          ...
          const ls = ["angle", "skewX", "skewY", "opacity"];
          const ol = {};
          ls.forEach((key) => {
            ol[key] = ops[key];
            delete ops[key];
          ...
          const cacheTextImage = new Image();
          cacheTextImage.src = baseText.toDataURL();

          const rs = ["width", "height", "scaleX", "scaleY", "fontSize", "stroke", "strokeWidth"];
          rs.forEach((key) => {
            ops[key] = (ops[key] || 1) * 2;
          });

          const textImage = new fabric.Text(safeText, ops);
          if (textImage.width === 0 || textImage.height === 0) {
            console.warn("⏳ Waiting for font to render (text-fx large)...");
            return setTimeout(() => lumise.objects["text-fx"](ops, callback), 100);
          }

          const cacheTextImageLarge = new Image();
          ...
          // ✅ Move everything INSIDE this callback
          fabric.Image.fromURL(textImage.toDataURL(), function (img) {
            if (!img || img.width === 0 || img.height === 0) {
              console.warn("❌ Invalid image rendered — retrying...");
              return setTimeout(() => lumise.objects["text-fx"](ops, callback), 100);
            }

            const cdata = lumise.fn.bridgeText(img._element, ops.bridge);
            ...
            // ✅ Use a closure-bound function so `img` is scoped
            img.set({
              clipTo: function (ctx) {
                return lumise.objects.clipto(ctx, this); // ✅ use `img` directly from closure scope
              }
            });
            ...
            if (ops.filters && ops.filters.length > 0) {
              const stage = lumise.stage();
              ops.filters.forEach((fil, ind) => {
                if (fil.color) {
                  ops.filters[ind] = new fabric.Image.filters.Tint({
                    color: fil.color,
                  });
              ...
              img.set("filters", ops.filters);
              img.applyFilters(stage.canvas.renderAll.bind(stage.canvas));
            }

            const _w = ops.width;
            const _h = ops.height;

            img.setSrc(cdata, function () {
              img.set({ height: _h, width: _w });
              lumise.stack.save();
              return callback(img);
            });
          });
                ...
                if (lumise.objects.lumise[data.objects[i].type]) {
                  ...
                  var do_add = function () {
                    ...
                    if (
                      ...
                      typeof data.objects[i].text === "string" &&
      ...
      create_canvas: function (stage, img) {
        ...
        */
        ...
        stage.limit_zone = new fabric.Rect({
          ...
          visible: false,
          radius: 0,
          rx: 0, // 0 is sharp corner, use 'radius' for round corner. 
          ry: 0, //'radius' variable is calculated above
        });
  ...
  //A1DotGrid
  fabric.A1DotGrid = fabric.util.createClass(fabric.Group, {
    type: 'a1_dotgrid',
  ...
  fabric.A1DotGrid.fromObject = function (object, callback) {
    console.log("🌀 Rehydrating A1DotGrid", object);

    fabric.util.enlivenObjects(object.objects, function (enlivenedObjects) {
      const options = fabric.util.object.clone(object);
      delete options.objects;

      const group = new fabric.A1DotGrid(enlivenedObjects, options);
      callback && callback(group);
    });
  };

  // ✅ This is the key to make Lumise recognize the type
  fabric['a1_dotgrid'] = fabric.A1DotGrid;

  //A1Box
  fabric.A1Box = fabric.util.createClass(fabric.Group, {
    type: 'a1_box',
  ...
  fabric.A1Box.fromObject = function (object, callback) {
    console.log("🌀 Rehydrating A1Box", object);

    fabric.util.enlivenObjects(object.objects, function (enlivenedObjects) {
      const options = fabric.util.object.clone(object);
      delete options.objects;

      const group = new fabric.A1Box(enlivenedObjects, options);
      callback && callback(group);
    });
  };

  // ✅ Register this type with Fabric so Lumise can deserialize it
  fabric['a1_box'] = fabric.A1Box;
    ...
    try {
      if (
        ...
        fabric.Group.fromObject = function (object, callback) {
          ...
          if (object.object_type === 'a1_dotgrid') {
            const group = createOrUpdateDotGrid(object.metadata, true);
            callback?.(group);
            return group;
          }

          if (object.object_type === 'a1_box') {
            const group = createOrUpdateA1Box(object.metadata, true);
            callback?.(group);
            return group;
          }
  ...
  function injectA1ModalsIfMissing() {
    if (!document.getElementById('a1_line_form')) {
      const html = `
        <div id="a1_modal_overlay" style="display: none; position: fixed; inset: 0; background: rgba(0, 0, 0, 0.5); z-index: 99999; pointer-events: none;"></div>
        ...
        <div id="a1_edit_form" style="display: none; position: fixed; top: 100px; left: 5%; width: 320px; background: #fff; border: 1px solid #ccc; padding: 16px; border-radius: 8px; z-index: 10000; box-shadow: 0 0 15px rgba(0,0,0,0.3); font-family: sans-serif;">
          ...
          <div style="display: flex; align-items: center; margin-bottom: 10px;">
            ...
            <input type="range" id="a1_edit_thickness" min="0.1" max="10" step="0.1" value="2" style="flex: 1;">
        ...
        <div id="a1_dotgrid_form" style="display: none; position: fixed; top: 100px; left: 5%; width: 320px; background: #fff; border: 1px solid #ccc; padding: 16px; border-radius: 8px; z-index: 10000; box-shadow: 0 0 15px rgba(0,0,0,0.3); font-family: sans-serif;">
        <h4 style="margin-top: 0;">Edit Dot Grid</h4>

        <div style="display: flex; align-items: center; margin-bottom: 10px;">
          <label style="width: 130px;">Rows</label>
          <input type="range" id="a1_edit_rows" min="1" max="50" value="5" style="flex: 1;">
          <span id="a1_rows_val" style="width: 30px; text-align: right;">5</span>
        </div>

        <div style="display: flex; align-items: center; margin-bottom: 10px;">
          <label style="width: 130px;">Columns</label>
          <input type="range" id="a1_edit_cols" min="1" max="50" value="5" style="flex: 1;">
          <span id="a1_cols_val" style="width: 30px; text-align: right;">5</span>
        </div>

        <div style="display: flex; align-items: center; margin-bottom: 10px;">
          <label style="width: 130px;">Spacing X (px)</label>
          <input type="range" id="a1_edit_spacing_x" min="5" max="100" value="20" style="flex: 1;">
          <span id="a1_spacing_x_val" style="width: 30px; text-align: right;">20</span>
        </div>

        <div style="display: flex; align-items: center; margin-bottom: 10px;">
          <label style="width: 130px;">Spacing Y (px)</label>
          <input type="range" id="a1_edit_spacing_y" min="5" max="100" value="20" style="flex: 1;">
          <span id="a1_spacing_y_val" style="width: 30px; text-align: right;">20</span>
        </div>

        <div style="display: flex; align-items: center; margin-bottom: 10px;">
          <label style="width: 130px;">Dot Radius (px)</label>
          <input type="range" id="a1_edit_dot_radius" min="1" max="10" value="2" style="flex: 1;">
          <span id="a1_dot_radius_val" style="width: 40px; text-align: right;">2</span>
        </div>

        <div style="display: flex; align-items: center; margin-bottom: 10px;">
          <label style="width: 130px;">Dot Color</label>
          <input type="color" id="a1_edit_dot_color" value="#000000">
        </div>

        <div style="margin-top: 15px; text-align: right;">
          <button id="a1_delete_dotgrid" class="small-button delete-button">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/></svg>
            Trash
          </button>
          <button id="a1-duplicate-dotgrid" class="small-button duplicate-button">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M8 2h8v8H8V2zm8 14h8v8h-8v-8z"/></svg>
            Copy
          </button>
          <button id="a1_cancel_dotgrid_edit" class="small-button done-button">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z"/></svg>
            Done
          </button>
        </div>
      </div>
      <div id="a1_box_form" style="display: none; position: fixed; top: 100px; left: 5%; width: 320px; background: #fff; border: 1px solid #ccc; padding: 16px; border-radius: 8px; z-index: 10000; box-shadow: 0 0 15px rgba(0,0,0,0.3); font-family: sans-serif;">
        <h4 style="margin-top: 0;">Edit Box</h4>

        <div style="display: flex; align-items: center; margin-bottom: 10px;">
          <label style="width: 130px;">Width (px)</label>
          <input type="range" id="a1_box_width" min="10" max="800" value="100" style="flex: 1;">
          <span id="a1_box_width_val" style="width: 40px; text-align: right;">100</span>
        </div>

        <div style="display: flex; align-items: center; margin-bottom: 10px;">
          <label style="width: 130px;">Height (px)</label>
          <input type="range" id="a1_box_height" min="10" max="800" value="100" style="flex: 1;">
          <span id="a1_box_height_val" style="width: 40px; text-align: right;">100</span>
        </div>

        <div style="display: flex; align-items: center; margin-bottom: 10px;">
          <label style="width: 130px;">Border Thickness</label>
          <input type="range" id="a1_box_thickness" min="1" max="20" value="2" style="flex: 1;">
          <span id="a1_box_thickness_val" style="width: 30px; text-align: right;">2</span>
        </div>

        <div style="display: flex; align-items: center; margin-bottom: 10px;">
          <label style="width: 130px;">Border Color</label>
          <input type="color" id="a1_box_color" value="#000000">
          <span id="a1_box_color_val" style="margin-left: 10px;">#000000</span>
        </div>

        <div style="margin-top: 15px; text-align: right;">
          <button id="a1_delete_box" class="small-button delete-button">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/></svg>
            Trash
          </button>
          <button id="a1-duplicate-box" class="small-button duplicate-button">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M8 2h8v8H8V2zm8 14h8v8h-8v-8z"/></svg>
            Copy
          </button>
          <button id="a1_cancel_box_edit" class="small-button done-button">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z"/></svg>
            Done
          </button>
        </div>
      ...
      <div id="a1_checkboxes_form" style="display: none; position: fixed; top: 100px; left: 5%; width: 320px; background: #fff; border: 1px solid #ccc; padding: 16px; border-radius: 8px; z-index: 10000; box-shadow: 0 0 15px rgba(0,0,0,0.3); font-family: sans-serif;">
          <h4 style="margin-top: 0;">Edit Set of Checkboxes</h4>
          <div style="display: flex; align-items: center; margin-bottom: 10px;">
          ...
          <div style="display: flex; align-items: center; margin-bottom: 10px;">
            ...
            <input type="range" id="a1_checkboxes_width" min="10" max="100" step="1" value="10" style="flex: 1;">
      ...
      style.textContent = `
        ...
        .duplicate-button {
          background-color: #008CBA; /* Example blue color */
          color: white;
        }
        ...
        .table-container {
          ...
          border-radius: 4px;
          overflow: hidden; /* To contain the border-radius of the inner elements */
        }
        ...
        .table-row {
          ...
          padding: 8px 12px;
          align-items: start; /* Align items to the top to accommodate wrapping text */
        }
        ...
        .table-cell {
          padding: 6px 0;
          overflow: visible; /* Allow content to be visible even if it wraps */
        ...
        .table-cell:last-child {
          ...
          white-space: nowrap; /* Keep the button on a single line */
          overflow: hidden; /* Hide overflow for the button cell if it gets too wide */
          text-overflow: ellipsis; /* Show ellipsis if the button cell overflows */
        }
    ...
    if (!document.getElementById('a1_dotted_form')) {
      const dottedHTML = `
        <div id="a1_edit_dotted_form" style="display: none; position: fixed; top: 100px; left: 5%; width: 320px; background: #fff; border: 1px solid #ccc; padding: 16px; border-radius: 8px; z-index: 10000; box-shadow: 0 0 15px rgba(0,0,0,0.3); font-family: sans-serif;">
          <h4 style="margin-top: 0;">Edit Dotted Grid</h4>

          <div style="display: flex; align-items: center; margin-bottom: 10px;">
            <label style="width: 130px;">Rows</label>
            <input type="range" id="a1_edit_dotted_rows" min="1" max="100" value="4" style="flex: 1;">
            <span id="a1_edit_dotted_rows_val" style="width: 30px; text-align: right;">4</span>
          </div>

          <div style="display: flex; align-items: center; margin-bottom: 10px;">
            <label style="width: 130px;">Row Spacing</label>
            <input type="range" id="a1_edit_dotted_row_spacing" min="5" max="100" value="10" style="flex: 1;">
            <span id="a1_edit_dotted_row_spacing_val" style="width: 30px; text-align: right;">10</span>
          </div>

          <div style="display: flex; align-items: center; margin-bottom: 10px;">
            <label style="width: 130px;">Grid Width</label>
            <input type="range" id="a1_edit_dotted_width" min="20" max="800" value="100" style="flex: 1;">
            <span id="a1_edit_dotted_width_val" style="width: 40px; text-align: right;">100</span>
          </div>

          <div style="display: flex; align-items: center; margin-bottom: 10px;">
            <label style="width: 130px;">Column Spacing</label>
            <input type="range" id="a1_edit_dotted_spacing" min="5" max="100" value="10" style="flex: 1;">
            <span id="a1_edit_dotted_spacing_val" style="width: 30px; text-align: right;">10</span>
          </div>

          <div style="display: flex; align-items: center; margin-bottom: 10px;">
            <label style="width: 130px;">Dot Radius</label>
            <input type="range" id="a1_edit_dotted_radius" min="1" max="20" value="2" style="flex: 1;">
            <span id="a1_edit_dotted_radius_val" style="width: 30px; text-align: right;">2</span>
          </div>

          <div style="display: flex; align-items: center; margin-bottom: 10px;">
            <label style="width: 130px;">Dot Color</label>
            <input type="color" id="a1_edit_dotted_color" value="#000000">
          </div>

          <div style="margin-top: 15px; text-align: right;">
            <button id="a1_delete_dotted" style="margin-left: 10px;">Delete</button>
            <button id="a1_cancel_dotted" style="margin-left: 10px;">Cancel</button>
          </div>
        </div>
      `;
      const container = document.createElement('div');
      container.innerHTML = dottedHTML;
      document.body.appendChild(container);
      const style = document.createElement('style');
      style.textContent = `
        #a1_modal_overlay,
  ...
  function initA1Designer(canvas) {

    injectA1ModalsIfMissing();

    function createOrUpdateLineset(meta, isEdit = false, dragDirection = 'center', positionOverrideX = null, positionOverrideY = null) {
      const canvas = lumise.stage().canvas;
      ...
      // Determine origins based on drag direction
      let originX = 'left';
      let originY = 'top';

      if (dragDirection === 'right') originX = 'left';
      else if (dragDirection === 'left') originX = 'right';

      if (dragDirection === 'bottom') originY = 'top';
      else if (dragDirection === 'top') originY = 'bottom';

      // Create lines with adjusted positions
      for (let i = 0; i < meta.numLines; i++) {
        ...
        if (dragDirection === 'top') {
          // Add lines upward from bottom
          y = -meta.spacing * (meta.numLines - 1 - i);
        } else if (dragDirection === 'bottom') {
          // Add lines downward from top
          y = i * meta.spacing;
        } else {
          // Balanced from center
          y = (i - (meta.numLines - 1) / 2) * meta.spacing;
        }

        lines.push(new fabric.Line([x1, y, x2, y], {
          stroke: meta.color,
      ...
      // Default position if not editing
      const limit = lumise.stage().limit_zone;
      const defaultX = limit.left + limit.width / 2;
      const defaultY = limit.top + limit.height / 2;

      const group = new fabric.Group(lines, {
        left: window._a1_active_group?.left || defaultX,
        top: window._a1_active_group?.top || defaultY,
        originX,
        originY,
        name: 'a1_lineset',
      ...
      // Optional manual position override
      if (positionOverrideX !== null) {
        group.set({ left: positionOverrideX });
      }
      if (positionOverrideY !== null) {
        group.set({ top: positionOverrideY });
      }
    ...
    // function createOrUpdateDotGrid(meta, isEdit = false) {

    //   console.log('[Update DotGrid]', meta);

    //   const canvas = lumise.stage().canvas;

    //   // Remove previous dotgrid group if any
    //   if (window._a1_active_group && canvas.contains(window._a1_active_group)) {
    //     canvas.remove(window._a1_active_group);
    //   }
    ...
    //       const x = col * meta.spacingX;
    //       const y = row * meta.spacingY;

    //       const dot = new fabric.Circle({
    //         radius: meta.radius,
    //         fill: meta.color, // live edit color applied here
    //         left: x,
    //         top: y,
    //         originX: 'center',
    //         originY: 'center',
    ...
    //     originY: 'center',
    //     name: 'a1_dotgrid',
    //     id: 'a1_dotgrid_' + Date.now(),
    //     metadata: meta,
    //     object_type: 'a1_dotgrid',
    //     object_id: 'a1_dotgrid_' + Date.now(),
    //     selectable: true,
    //     evented: true
    //   });
    ...
    //   // ⬅️ attach custom handles for rows/cols manipulation
    //   attachHandlesToLineset(group);
    ...
    function createOrUpdateDotGrid(meta, isEdit = false, dragDirection = 'center', positionOverrideX = null, positionOverrideY = null) {
      const canvas = lumise.stage().canvas;
      console.log('[Update DotGrid]', meta, 'Direction:', dragDirection);
      ...
      // Set group origin based on drag direction
      let originX = 'left';
      let originY = 'top';
      ...
      // Place dots with respect to drag direction
      for (let row = 0; row < meta.rows; row++) {
        for (let col = 0; col < meta.cols; col++) {
          let x, y;
          ...
          if (dragDirection === 'right') {
            x = col * meta.spacingX; // From 0 expanding right
          } else if (dragDirection === 'left') {
            x = -col * meta.spacingX; // From 0 expanding left
          } else {
            x = (col - (meta.cols - 1) / 2) * meta.spacingX; // Centered
          }
          ...
          if (dragDirection === 'bottom') {
            ...
            y = (row - (meta.rows - 1) / 2) * meta.spacingY;
          }
      ...
      // Default position if not editing
      const limit = lumise.stage().limit_zone;
      const defaultX = limit.left + 10;
      const defaultY = limit.top + 10;

      // Maintain current position if editing, otherwise use default
      const groupLeft = positionOverrideX !== null
        ? positionOverrideX
        : (window._a1_active_group?.left || defaultX);
      const groupTop = positionOverrideY !== null
        ? positionOverrideY
        : (window._a1_active_group?.top || defaultY);
    ...
    function createOrUpdateA1Box(meta, isEdit = false) {
      const canvas = lumise.stage().canvas;
      ...
      const { width, height, color, thickness } = meta;

      // Position in center of printable zone
      const limit = lumise.stage().limit_zone;
      const x = limit.left + limit.width / 2;
      const y = limit.top + limit.height / 2;
      ...
      // Track globally
      window._a1_active_group = rect;

      // ⬅️ attach custom handles for rows/cols manipulation
      attachHandlesToLineset(rect);


      return rect;
    }

    function createOrUpdateCheckboxes(meta, isEdit = false) {
      console.log(meta);
      const canvas = lumise.stage().canvas;
      ...
      for (let i = 0; i < meta.numBoxes; i++) {
        checkboxes.push(new fabric.Rect({
          left: 0,
          top: i * (meta.spacing + meta.width), // space between top edges
          width: meta.width,
          height: meta.width,
          fill: 'transparent',
          stroke: meta.color,
          strokeWidth: meta.thickness || 1,
          originX: 'center',
          originY: 'center',
          selectable: false,
          evented: false
      ...
      const group = new fabric.Group(checkboxes, {
        ...
        fill: 'transparent',
        name: 'a1_checkboxes',
        id: 'a1_checkboxes_' + now,
        metadata: meta,
        object_type: 'a1_checkboxes',
        object_id: 'a1_checkboxes_' + now,
        selectable: true,
        evented: true
      });
    ...
    //live editing of a1_lineset lines

    let a1_input_timer = null;
    let handle_direction = 'center';


    // setInputsFromMeta is used to set the values while dragging the handles.
    function setInputsFromMeta(meta) {
      if (!meta) return;

      // const clamp = (val, min = 1) => Math.max(min, parseInt(val, 10) || min);
      const clamp = (val, min = 1) => Math.max(min, parseFloat(val) || min).toFixed(2);


      document.getElementById("a1_edit_num_lines").value = clamp(meta.numLines);
      document.getElementById("a1_edit_spacing").value = clamp(meta.spacing);
      document.getElementById("a1_edit_width").value = clamp(meta.width);
      document.getElementById("a1_edit_color").value = meta.color || "#000000";
      document.getElementById("a1_edit_thickness").value = clamp(meta.thickness);

      // Optional: update displayed label values if present
      const thicknessVal = document.getElementById("a1_thickness_val");
      if (thicknessVal) thicknessVal.textContent = clamp(meta.thickness);
      ...
      ['num_lines', 'spacing', 'width', 'thickness'].forEach(field => {
        const label = document.getElementById(`a1_${field}_val`);
        const metaKey = fieldMap[field];
        if (label && meta[metaKey] != null) {
          if (field === 'num_lines') {
            label.textContent = meta[metaKey];
          } else {
            label.textContent = clamp(meta[metaKey]);
          }
        }
      });
    ...
    function getMetaFromInputs() {
      return {
        numLines: Math.max(1, parseInt(document.getElementById("a1_edit_num_lines")?.value || "0", 10)),
        spacing: Math.max(1, parseInt(document.getElementById("a1_edit_spacing")?.value || "0", 10)),
        width: Math.max(1, parseInt(document.getElementById("a1_edit_width")?.value || "0", 10)),
        color: document.getElementById("a1_edit_color")?.value || "#000000",
        thickness: Math.max(0.1, parseInt(document.getElementById("a1_edit_thickness")?.value || "2", 10))
      };
    }

    function applyLinesetUpdate() {
      const meta = getMetaFromInputs();

      if (window._a1_active_group) {
        createOrUpdateLineset(meta, true, handle_direction); // isEdit = true
      }
    }
    ...
    ['num_lines', 'spacing', 'width', 'color', 'thickness'].forEach(field => {
      ...
      input?.addEventListener('input', () => {
        ...
        clearTimeout(a1_input_timer);
        a1_input_timer = setTimeout(applyLinesetUpdate, 200);
      });
    });

    //old live editing code
    // ['num_lines', 'spacing', 'width', 'color', 'thickness'].forEach(field => {
    //   const input = document.getElementById(`a1_edit_${field}`);
    //   const output = document.getElementById(`a1_${field}_val`);
    ...
    function getDotGridMetaFromInputs() {
      return {
        rows: Math.max(1, parseInt(document.getElementById("a1_edit_rows")?.value || "5", 10)),
        cols: Math.max(1, parseInt(document.getElementById("a1_edit_cols")?.value || "5", 10)),
        spacingX: Math.max(1, parseInt(document.getElementById("a1_edit_spacing_x")?.value || "20", 10)),
        spacingY: Math.max(1, parseInt(document.getElementById("a1_edit_spacing_y")?.value || "20", 10)),
        radius: Math.max(1, parseInt(document.getElementById("a1_edit_dot_radius")?.value || "2", 10)),
        color: document.getElementById("a1_edit_color")?.value || "#000000"
      };
    }

    function applyDotGridUpdate() {
      const meta = getDotGridMetaFromInputs();

      if (window._a1_active_group && window._a1_active_group.name === 'a1_dotgrid') {
        createOrUpdateDotGrid(meta, true); // true = isEdit
      }
    }

    ['rows', 'cols', 'spacing_x', 'spacing_y', 'dot_radius'].forEach(field => {
      const input = document.getElementById(`a1_edit_${field}`);
      const output = document.getElementById(`a1_${field}_val`);
      ...
      input?.addEventListener('input', () => {
        ...
        clearTimeout(a1_input_timer);
        a1_input_timer = setTimeout(applyDotGridUpdate, 200);
      });
    });
    ...
    //new event listeners for dot grids

    //form id: a1_dotgrid_form

    //input box IDs for dot grids: 
    // a1_edit_rows, a1_edit_cols, a1_edit_spacing_x, a1_edit_spacing_y, a1_edit_dot_radius, a1_edit_dot_color

    // span ids for values:
    // a1_rows_val, a1_cols_val, a1_spacing_x_val, a1_spacing_y_val, a1_dot_radius_val, 
    ...
    //       window._a1_active_group
    //     ) {
    //       createOrUpdateDotGrid(meta, true); // isEdit = true
    //     }
    //   });
    // });
    ...
    a1_dotgrid_colorInput?.addEventListener("input", () => {
      _a1_dot_color = a1_dotgrid_colorInput.value;

      if (window._a1_active_group && window._a1_active_group._objects) {
        window._a1_active_group._objects.forEach(dot => {
          dot.set("fill", _a1_dot_color);
        });
        lumise.stage().canvas.renderAll();
      }
    });

    //Live Editing of a1_box

    ['width', 'height', 'thickness', 'color'].forEach(field => {
      const input = document.getElementById(`a1_box_${field}`);
      const output = document.getElementById(`a1_box_${field}_val`);
      ...
      input?.addEventListener('input', () => {
        ...
        const meta = {
          width: parseInt(document.getElementById("a1_box_width")?.value || "0", 10),
          height: parseInt(document.getElementById("a1_box_height")?.value || "0", 10),
          thickness: parseInt(document.getElementById("a1_box_thickness")?.value || "1", 10),
          color: document.getElementById("a1_box_color")?.value || "#000000"
        };
        ...
        if (
          ...
          createOrUpdateA1Box(meta, true); // isEdit = true
        }
      });
    });

    //live editing of a1_checkboxes
    // Live editing for a1_checkboxes

    let a1_checkbox_input_timer = null;

    function getCheckboxMetaFromInputs() {
      return {
        numBoxes: Math.max(1, parseInt(document.getElementById("a1_checkboxes_num_boxes")?.value || "0", 10)),
        spacing: Math.max(1, parseInt(document.getElementById("a1_checkboxes_spacing")?.value || "0", 10)),
        width: Math.max(1, parseInt(document.getElementById("a1_checkboxes_width")?.value || "0", 10)),
        color: document.getElementById("a1_checkboxes_color")?.value || "#000000",
        thickness: Math.max(1, parseInt(document.getElementById("a1_checkboxes_thickness")?.value || "1", 10))
      };
    }

    function applyCheckboxesUpdate() {
      const meta = getCheckboxMetaFromInputs();

      if (window._a1_active_group) {
        createOrUpdateCheckboxes(meta, true); // isEdit = true
      }
    }

    ['num_boxes', 'spacing', 'width', 'color', 'thickness'].forEach(field => {
      const input = document.getElementById(`a1_checkboxes_${field}`);
      const output = document.getElementById(`a1_checkboxes_${field}_val`);
      ...
      input?.addEventListener('input', () => {
        ...
        clearTimeout(a1_checkbox_input_timer);
        a1_checkbox_input_timer = setTimeout(applyCheckboxesUpdate, 200);
      });
    });
    ...
    // Show modal
    // initial creation of a1_lineset
    document.getElementById("a1_lines")?.addEventListener("click", function () {
      ...
      // 🔁 Reattach new handles
      attachHandlesToLineset(group);

      a1_showDimensions(window._a1_active_group);



    });

    // Initial creation of dot grids

    document.getElementById("a1_dotgrid")?.addEventListener("click", function () {
      const defaultMeta = {
        rows: 5,
        cols: 5,
        spacingX: 20,
      ...
      // Prefill modal form (optional UI)
      document.getElementById("a1_edit_rows").value = defaultMeta.rows;
      document.getElementById("a1_edit_cols").value = defaultMeta.cols;
      document.getElementById("a1_edit_spacing_x").value = defaultMeta.spacingX;
      document.getElementById("a1_edit_spacing_y").value = defaultMeta.spacingY;
      document.getElementById("a1_edit_dot_radius").value = defaultMeta.radius;
      document.getElementById("a1_edit_dot_color").value = _a1_dot_color;

      // Show modal (if applicable)
      document.getElementById("a1_modal_overlay").style.display = "none";
      document.getElementById("a1_dotgrid_form").style.display = "block";

      // ⬇️ Use the refactored function
      createOrUpdateDotGrid(defaultMeta, false, 'center');
    });
    ...
    //   // Prefill modal form (if you have one later)
    //   //form id: a1_dotgrid_form
    ...
    //   for (let row = 0; row < defaultMeta.rows; row++) {
    //     for (let col = 0; col < defaultMeta.cols; col++) {
    //       const x = col * defaultMeta.spacingX;
    //       const y = row * defaultMeta.spacingY;

    //       const dot = new fabric.Circle({
    //         radius: defaultMeta.radius,
    //         fill: defaultMeta.color,
    //         left: x,
    //         top: y,
    //         originX: 'center',
    //         originY: 'center',
    ...
    //   const group = new fabric.A1DotGrid(dots, {
    //     left: limit.left + limit.width / 2,
    //     top: limit.top + limit.height / 2,
    //     fill: _a1_dot_color,
    //     originX: 'center',
    ...
    // initial creation of a1_box
    document.getElementById("a1_box")?.addEventListener("click", function () {
      const defaultMeta = {
        width: 100,
        height: 60,
        thickness: 2,
      ...
      // Pre-fill the box edit modal with default values
      document.getElementById("a1_box_width").value = defaultMeta.width;
      document.getElementById("a1_box_height").value = defaultMeta.height;
      document.getElementById("a1_box_thickness").value = defaultMeta.thickness;
      document.getElementById("a1_box_color").value = defaultMeta.color;

      // Update display values
      document.getElementById("a1_box_width_val").textContent = defaultMeta.width;
      document.getElementById("a1_box_height_val").textContent = defaultMeta.height;
      document.getElementById("a1_box_thickness_val").textContent = defaultMeta.thickness;
      document.getElementById("a1_box_color_val").textContent = defaultMeta.color;

      // Show modal
      document.getElementById("a1_modal_overlay").style.display = "block";
      document.getElementById("a1_box_form").style.display = "block";

      // ✅ Actually create the rectangle box
      const canvas = lumise.stage().canvas;
      const limit = lumise.stage().limit_zone;
      const x = limit.left + limit.width / 2;
      const y = limit.top + limit.height / 2;
      ...
      const box = new fabric.Rect({
        ...
        fill: 'transparent',
        stroke: defaultMeta.color,
        strokeWidth: defaultMeta.thickness,
        originX: 'center',
        originY: 'center',
        name: 'a1_box',
        id: 'a1_box_' + Date.now(),
      ...
      // Optionally control resize/move handles
      box.setControlsVisibility({
        mt: false, mb: false, ml: false, mr: false,
        bl: false, br: false, tl: false, tr: false, mtr: false
      });

      box.hasControls = false;
      box.hasBorders = true;
      ...
      window._a1_active_group = box;

      // 🔁 Reattach new handles
      attachHandlesToLineset(box);


    });

    document.getElementById("a1_checkboxes")?.addEventListener("click", function () {
      const defaultMeta = {
        numBoxes: 3,
        spacing: 20,
        width: 10,
      ...
      // Pre-fill input values in the editing form
      document.getElementById("a1_checkboxes_num_boxes").value = defaultMeta.numBoxes;
      document.getElementById("a1_checkboxes_num_boxes_val").textContent = defaultMeta.numBoxes;

      document.getElementById("a1_checkboxes_spacing").value = defaultMeta.spacing;
      document.getElementById("a1_checkboxes_spacing_val").textContent = defaultMeta.spacing;

      document.getElementById("a1_checkboxes_width").value = defaultMeta.width;
      document.getElementById("a1_checkboxes_width_val").textContent = defaultMeta.width;

      document.getElementById("a1_checkboxes_thickness").value = defaultMeta.thickness;
      document.getElementById("a1_checkboxes_thickness_val").textContent = defaultMeta.thickness;

      document.getElementById("a1_checkboxes_color").value = defaultMeta.color;

      // Show modal
      document.getElementById("a1_modal_overlay").style.display = "block";
      document.getElementById("a1_checkboxes_form").style.display = "block";

      // Create checkbox group
      const boxes = [];
      for (let i = 0; i < defaultMeta.numBoxes; i++) {
        const box = new fabric.Rect({
          left: 0,
          top: i * defaultMeta.spacing,
          width: defaultMeta.width,
          height: defaultMeta.width,
          fill: 'transparent',
          strokeWidth: defaultMeta.thickness,
          stroke: defaultMeta.color,
          selectable: false,
      ...
      const group = new fabric.Group(boxes, {
        left: limit.left + limit.width / 2,
        top: limit.top + limit.height / 2,
        originX: 'center',
        originY: 'center',
      ...
      // Set active group reference
      window._a1_active_group = group;

      // Attach custom handles for editing, if needed
      attachHandlesToLineset(group);
    });
    ...
    //new event listener for mouse down
    canvas.on('mouse:down', function (e) {
      const target = e.target;
      const editForm = document.getElementById('a1_edit_form');
      const dotForm = document.getElementById('a1_dotgrid_form'); // dotgrid edit modal
      const boxForm = document.getElementById('a1_box_form'); // dotgrid edit modal
      const checkboxesForm = document.getElementById('a1_checkboxes_form'); // checkboxes edit modal
      const overlay = document.getElementById("a1_modal_overlay");
      ...
      checkboxesForm.style.display = "none";
      overlay.style.display = "none";

      if (target && target.metadata) {
        const meta = target.metadata;

        if (target.name === 'a1_lineset') {
          window._a1_active_group = target;
          ...
          document.getElementById("a1_edit_width").value = meta.width || 100;
          document.getElementById("a1_edit_color").value = meta.color || "#000000";
          document.getElementById("a1_edit_thickness").value = meta.thickness || 2;

          // Optional: update value displays for sliders
          document.getElementById("a1_thickness_val").textContent = meta.thickness || 2;

          editForm.style.display = "block";
          // overlay.style.display = "block";
          console.log("🛠️ Edit modal opened for LineSet:", meta);

        } else if (target.name === 'a1_dotgrid') {
          //form id: a1_dotgrid_form
          ...
          document.getElementById("a1_edit_rows").value = meta.rows || 5;
          document.getElementById("a1_edit_cols").value = meta.cols || 5;
          document.getElementById("a1_edit_spacing_x").value = meta.spacingX || 20;
          document.getElementById("a1_edit_spacing_y").value = meta.spacingY || 20;
          document.getElementById("a1_edit_dot_radius").value = meta.radius || 2;
          document.getElementById("a1_edit_dot_color").value = _a1_dot_color;

          // Optional: update slider value displays
          document.getElementById("a1_dot_radius_val").textContent = meta.radius || 2;

          dotForm.style.display = "block";
          overlay.style.display = "block";
          console.log("🛠️ Edit modal opened for DotGrid:", meta);

        } else if (target.name === 'a1_box') {
          window._a1_active_group = target;
          const meta = target.metadata;

          document.getElementById("a1_box_width").value = meta.width || 100;
          document.getElementById("a1_box_height").value = meta.height || 60;
          document.getElementById("a1_box_thickness").value = meta.thickness || 2;
          document.getElementById("a1_box_color").value = meta.color || "#000000";

          // Update output spans
          document.getElementById("a1_box_width_val").textContent = meta.width || 100;
          document.getElementById("a1_box_height_val").textContent = meta.height || 60;
          document.getElementById("a1_box_thickness_val").textContent = meta.thickness || 2;
          document.getElementById("a1_box_color_val").textContent = meta.color || "#000000";

          document.getElementById("a1_box_form").style.display = "block";
          document.getElementById("a1_modal_overlay").style.display = "block";
          console.log("🛠️ Edit modal opened for Box:", meta);

        } else if (target.name === 'a1_checkboxes') {
          window._a1_active_group = target;
          const meta = target.metadata;
          ...
          // Pre-fill input values in the editing form
          document.getElementById("a1_checkboxes_num_boxes").value = meta.numBoxes || 3;
          document.getElementById("a1_checkboxes_num_boxes_val").textContent = meta.numBoxes || 3;

          document.getElementById("a1_checkboxes_spacing").value = meta.spacing || 20;
          document.getElementById("a1_checkboxes_spacing_val").textContent = meta.spacing || 20;

          document.getElementById("a1_checkboxes_width").value = meta.width || 10;
          document.getElementById("a1_checkboxes_width_val").textContent = meta.width || 10;

          document.getElementById("a1_checkboxes_thickness").value = meta.thickness || 2;
          document.getElementById("a1_checkboxes_thickness_val").textContent = meta.thickness || 2;

          document.getElementById("a1_checkboxes_color").value = meta.color || "#000000";

          document.getElementById("a1_checkboxes_form").style.display = "block";
          document.getElementById("a1_modal_overlay").style.display = "block";
          console.log("🛠️ Edit modal opened for Checkboxes:", meta);
        }
    ...
    document.getElementById("a1_modal_overlay").onclick = function () {
      document.getElementById("a1_edit_form").style.display = "none";
      document.getElementById("a1_dotgrid_form").style.display = "none";
      document.getElementById("a1_box_form").style.display = "none";
      document.getElementById("a1_modal_overlay").style.display = "none";
    };

    //cancel edit form
    document.getElementById("a1_cancel_edit").onclick = function () {
      document.getElementById("a1_edit_form").style.display = "none";
      document.getElementById("a1_modal_overlay").style.display = "none";
    };

    //delete function
    function a1_deleteSelectedObject() {
    ...
    //new delete listener 
    document.getElementById("a1_delete_group").onclick = a1_deleteSelectedObject;
    ...
    function duplicate_a1_lineset() {
      ...
      // Create a shallow copy of metadata and assign new ID
      const newMeta = {
        ...originalMeta
      };
      ...
      // Create lines as per newMeta
      const lines = [];
      for (let i = 0; i < newMeta.numLines; i++) {
        lines.push(new fabric.Line(
          [0, i * newMeta.spacing, newMeta.width, i * newMeta.spacing],
          {
    ...
    document.getElementById('a1-duplicate-lineset').addEventListener('click', duplicate_a1_lineset);
    ...
    // new duplicate dot grid
    document.getElementById('a1-duplicate-dotgrid').addEventListener('click', function () {
      const group = window._a1_active_group;

      if (!group || group.name !== 'a1_dotgrid' || !group.metadata) {
        alert("No active dotgrid to duplicate.");
        return;
      }

      const originalMeta = group.metadata;
      ...
      const limit = lumise.stage().limit_zone;
      const x = (group.left || limit.left) + 40;
      const y = (group.top || limit.top) + 40;

      const dots = [];

      for (let r = 0; r < newMeta.rows; r++) {
        for (let c = 0; c < newMeta.cols; c++) {
          const cx = c * newMeta.spacingX;
          const cy = r * newMeta.spacingY;

          dots.push(new fabric.Circle({
            left: cx,
            top: cy,
            radius: newMeta.radius || 2,
            fill: newMeta.color || "#000000",
            originX: 'center',
            originY: 'center',
            selectable: false,
            evented: false
      ...
      const newGroup = new fabric.Group(dots, {
        left: x,
        top: y,
        fill: _a1_dot_color,
        originX: 'center',
        originY: 'center',
        name: 'a1_dotgrid',
        id: newMeta.object_id,
        metadata: newMeta,
        object_type: 'a1_dotgrid',
        object_id: newMeta.object_id,
        selectable: true,
        evented: true
      });
      ...
      window._a1_active_group = newGroup;

      alert('✅ Dotgrid duplicated!');
    });

    //duplicate a1_box
    document.getElementById('a1-duplicate-box').addEventListener('click', function () {
      if (!window._a1_active_group || window._a1_active_group.object_type !== 'a1_box') {
        alert("No active box to duplicate.");
        return;
      }

      const originalMeta = window._a1_active_group.metadata;

      // Shallow clone of metadata
      const newMeta = {
        ...originalMeta
      };

      const newId = 'a1_box_' + Date.now();
      newMeta.object_id = newId;
      ...
      alert('✅ Box duplicated!');
    });


    //old dot grid creation code
    document.getElementById("a1_dottedgrid")?.addEventListener("click", function () {
      const defaultMeta = {
        rows: 4,
        rowSpacing: 10,
        width: 100,
      ...
      // Set input values
      document.getElementById("a1_edit_dotted_rows").value = defaultMeta.rows;
      document.getElementById("a1_edit_dotted_row_spacing").value = defaultMeta.rowSpacing;
      document.getElementById("a1_edit_dotted_width").value = defaultMeta.width;
      document.getElementById("a1_edit_dotted_spacing").value = defaultMeta.dotSpacing;
      document.getElementById("a1_edit_dotted_radius").value = defaultMeta.dotRadius;
      document.getElementById("a1_edit_dotted_color").value = defaultMeta.dotColor;

      // ✅ Set display span values correctly
      document.getElementById("a1_edit_dotted_rows_val").textContent = defaultMeta.rows;
      document.getElementById("a1_edit_dotted_row_spacing_val").textContent = defaultMeta.rowSpacing;
      document.getElementById("a1_edit_dotted_width_val").textContent = defaultMeta.width;
      document.getElementById("a1_edit_dotted_spacing_val").textContent = defaultMeta.dotSpacing;
      document.getElementById("a1_edit_dotted_radius_val").textContent = defaultMeta.dotRadius;

      // Show modal
      document.getElementById("a1_modal_overlay").style.display = "block";
      document.getElementById("a1_edit_dotted_form").style.display = "block";

      // Create initial grid
      createOrUpdateDottedGrid(defaultMeta, true);
    });



    document.getElementById("a1_cancel_dotgrid_edit")?.addEventListener("click", () => {
      document.getElementById("a1_modal_overlay").style.display = "none";
      document.getElementById("a1_dotgrid_form").style.display = "none";
    });

    document.getElementById("a1_cancel_box_edit")?.addEventListener("click", () => {
      document.getElementById("a1_modal_overlay").style.display = "none";
      document.getElementById("a1_box_form").style.display = "none";
    });

    document.getElementById("a1_delete_dotgrid")?.addEventListener("click", () => {
      const group = window._a1_active_group;
      if (group && lumise.stage().canvas.contains(group)) {
        lumise.stage().canvas.remove(group);
        lumise.stage().canvas.renderAll();
      }
    ...
    document.getElementById("a1_delete_box")?.addEventListener("click", () => {
      const group = window._a1_active_group;
      if (group && lumise.stage().canvas.contains(group)) {
        lumise.stage().canvas.remove(group);
        lumise.stage().canvas.renderAll();
    ...
    const A1_HANDLE_SVGS = {
      tl: `<svg viewBox="0 0 24 24" fill="white"><path d="M3 6h18v2H3V6zm2 3h14l-1.5 12.5a2 2 0 0 1-2 1.5H8.5a2 2 0 0 1-2-1.5L5 9zM9 11v7h2v-7H9zm4 0v7h2v-7h-2z"/></svg>`,
      r: `<svg viewBox="0 0 24 24" fill="white"><path d="M10 17l5-5-5-5v10z"/></svg>`,
      b: `<svg viewBox="0 0 24 24" fill="white"><path d="M7 10l5 5 5-5H7z"/></svg>`,
      bl: `<svg viewBox="0 0 24 24" fill="white"><path d="M16 1H4a2 2 0 0 0-2 2v14h2V3h12V1zM20 5H8a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2zm0 18H8V7h12v16z"/></svg>`
    };
    ...
    function attachHandlesToLineset(group) {
      ...
      const handleSize = 12;
      const names = ['tl', 'bl', 'b', 'r'];

      names.forEach(pos => {
        // Create handle background
        const handleRect = new fabric.Rect({
          width: handleSize,
          height: handleSize,
          fill: '#FF6E00',
          stroke: 'red',
        ...
        // Choose symbol
        let symbol = '';
        if (pos === 'b') symbol = '\u25BD';           // × (bottom)
        if (pos === 'r') symbol = '\u25B7';           // × (right)
        if (pos === 'tl') symbol = '\u00D7';           // × (delete)
        if (pos === 'bl') symbol = '\u26AF';     // 📋 (clipboard - duplicate) \uD83D\uDCCB u25CC

        // Create symbol text
        const handleText = new fabric.Text(symbol, {
          fontSize: 12,
          fill: 'white',
          originX: 'center',
          originY: 'center',
        ...
        // Group rect and text
        const handleGroup = new fabric.Group([handleRect, handleText], {
          left: 0,
          top: 0,
          name: `a1_handle_${pos}`,
          originX: 'left',
        ...
        // Lock movement based on direction
        if (pos === 'r') {
          handleGroup.lockMovementX = false;
          handleGroup.lockMovementY = true;
        } else if (pos === 'b') {
          handleGroup.lockMovementY = false;
          handleGroup.lockMovementX = true;
        } else {
          handleGroup.lockMovementX = false;
          handleGroup.lockMovementY = false;
        }

        handleGroup.a1_parent_group = group;

        A1_HANDLES.push(handleGroup);
        canvas.add(handleGroup);
        handleGroup.bringToFront();
        handleGroup.setCoords();
      });
    ...
    //   const handleSize = 20;
    //   const iconSize = 16; // icon area target
    //   const names = ['tl', 'r', 'b', 'bl']; // bl = bottom-left

    //   names.forEach(pos => {
    //     const svgMarkup = A1_HANDLE_SVGS[pos];
    //     if (!svgMarkup) return;
    ...
    //       // Scale to fit iconSize (typically 16x16 inside 20x20 box)
    //       const scaleX = iconSize / options.width;
    //       const scaleY = iconSize / options.height;
    ...
    //       const handleGroup = new fabric.Group([handleRect, iconGroup], {
    //         left: 0,
    //         top: 0,
    //         name: `a1_handle_${pos}`,
    //         originX: 'left',
    ...
    //tooltip for handles
    const tooltipText = new fabric.Text('', {
      fontSize: 12,
      fill: '#ffffcc',
      backgroundColor: '#333',
      opacity: 0,
    ...
    canvas.add(tooltipText);
    tooltipText.bringToFront();

    canvas.on('mouse:over', function (e) {
      const target = e.target;
      if (target && target.name?.startsWith('a1_handle_')) {
        const pos = target.name.replace('a1_handle_', '');
        let label = '';

        switch (pos) {
          case 'tl': label = 'Delete'; break;
          case 'r': label = 'Drag Right'; break;
          case 'b': label = 'Drag Down'; break;
          case 'bl': label = 'Copy'; break;
          default: label = '';
        }
    ...
    canvas.on('mouse:move', function (e) {
      ...
      const currentLabel = tooltipText.text;

      let offsetX = 10;
      let offsetY = 10;

      // If it's the "Delete" label, position above the cursor
      if (currentLabel === 'Delete') {
        offsetY = -tooltipText.height - 10;  // shift upward
      }
    ...
    function updateHandlePositions(group) {
      ...
      positions.forEach(pos => {
        ...
        // ⛔ DO NOT reposition if this is the active handle
        if (activeHandle && activeHandle.name === handle.name) return;
    ...
    // Remove DImension Handles
    function removeDimensionText() {
      const canvas = lumise.stage().canvas; // Ensure canvas context is available here

      // --- ADD THIS LOGIC TO REMOVE THE DIMENSION TEXT OBJECT ---
      if (_a1_dimension_text_object) {
        canvas.remove(_a1_dimension_text_object); // Remove the fabric.Text object from the canvas
        _a1_dimension_text_object = null; // Set it to null to indicate it's no longer present
      }
      // --- END ADDITION ---

      canvas.renderAll(); // Ensure the canvas updates after removal
    }
    ...
    canvas.on('mouse:down', function (e) {
      ...
      if (target?.name?.startsWith('a1_handle_')) {
        ...
        if (target.a1_parent_group) {
          ...
          if (target.name === 'a1_handle_tl') {
            console.log('tl handle clicked')
            a1_deleteSelectedObject();
          }
          if (target.name === 'a1_handle_bl') {
            console.log('bl handle clicked')
            if ((group.name) === 'a1_lineset') {
              duplicate_a1_lineset();
            }

          }

          // 👉 RIGHT HANDLE for a1_dotgrid and a1_box
          if (target.name === 'a1_handle_r') {
            const pointer = canvas.getPointer(e.e);
            handle_direction = 'right';

            if (group.name === 'a1_dotgrid') {
              // a1_drag_origin = {
              //   initialX: pointer.x,
              //   initialRightEdge: bbox.left + bbox.width,
              //   initialCols: parseInt(document.getElementById('a1_edit_cols').value) || 5
              // };
              a1_drag_origin = {
                initialX: pointer.x,
                initialRightEdge: bbox.left + bbox.width,
                initialBottomEdge: bbox.top + bbox.height,
                initialCols: group.metadata?.cols || 5,
                initialRows: group.metadata?.rows || 5
              };

            }

            if (group.name === 'a1_box') { // [a1_box]
              a1_drag_origin = {
                x: pointer.x,
                groupLeft: group.left,
                groupWidth: group.width * group.scaleX,
                initialWidth: parseInt(document.getElementById('a1_box_width')?.value || '100', 10)
              };
            }
          ...
          if (target.name === 'a1_handle_l') {
            ...
            if (groupName === 'a1_dotgrid') {
              a1_drag_origin = {
                initialX: pointer.x,
                initialLeftEdge: bbox.left,
                initialCols: parseInt(document.getElementById('a1_edit_cols').value) || 5
              };
            } else if (groupName === 'a1_box') { // [a1_box]
              a1_drag_origin = {
                x: pointer.x,
                groupLeft: group.left,
                groupWidth: group.width * group.scaleX,
              ...
              // Default (e.g. a1_lineset)

              // a1_drag_origin = {
              //   x: pointer.x,
              //   rightEdge: group.left + group.getScaledWidth(), // ⬅️ Capture right edge anchor
              //   initialWidth: group.getScaledWidth()
              // };
          ...
          // 👉 TOP / BOTTOM HANDLE
          if (target.name === 'a1_handle_t') handle_direction = 'top';
          if (target.name === 'a1_handle_b') handle_direction = 'bottom';
          if (target.name === 'a1_handle_t' || target.name === 'a1_handle_b') {
            const pointer = canvas.getPointer(e.e);
            const direction = target.name === 'a1_handle_t' ? 'top' : 'bottom';
            ...
            if (group.name === 'a1_lineset') {
              a1_vertical_drag_origin = {
                ...
                direction,
                top: group.top // 👈 Store actual group's top
              };
            }
            ...
            if (group.name === 'a1_dotgrid') {
              a1_vertical_drag_origin = {
                initialY: pointer.y,
                initialTopEdge: bbox.top,
                initialBottomEdge: bbox.top + bbox.height,
                initialRows: parseInt(document.getElementById('a1_edit_rows')?.value || '5', 10),
                direction
              };
            }
            ...
            if (group.name === 'a1_box') { // [a1_box]
              a1_vertical_drag_origin = {
                y: pointer.y,
                groupTop: group.top,
                groupHeight: group.height * group.scaleY,
                direction,
                initialHeight: parseInt(document.getElementById('a1_box_height')?.value || '60', 10)
              };
            }
          }
        }
    ...
    //       if (target.name === 'a1_handle_r' && target.a1_parent_group?.name === 'a1_dotgrid') {
    //         const pointer = canvas.getPointer(e.e);
    //         const bbox = group.getBoundingRect(true);
    ...
    //       // 👉 LEFT HANDLE (Merged logic)
    //       if (target.name === 'a1_handle_l') {
    //         const pointer = canvas.getPointer(e.e);
    //         const groupName = target.a1_parent_group?.name;
    ...
    //       // 👉 TOP/BOTTOM HANDLE
    //       if ((target.name === 'a1_handle_t' || target.name === 'a1_handle_b') && target.a1_parent_group) {
    //         const pointer = canvas.getPointer(e.e);
    //         const group = target.a1_parent_group;
    //         const direction = target.name === 'a1_handle_t' ? 'top' : 'bottom';
    ...
    canvas.on('object:moving', function (e) {
      ...
      if ((obj?.name === 'a1_lineset') || (obj?.name === 'a1_dotgrid') || (obj?.name === 'a1_box')) {
      ...
      if (handleName === 'a1_handle_r') {
        ...
        switch (groupName) {
          ...
          case 'a1_lineset': {
            const newWidth = Math.max(pointer.x - bbox.left, 10);
            createOrUpdateLineset({
              ...group.metadata,
              width: newWidth
            }, true, 'right'); // 👈 Note the 'right' direction
          ...
          case 'a1_dotgrid': {
            ...
            const spacingX = meta?.spacingX || 20;
            const currentCols = meta?.cols || 5;

            const delta = pointer.x - (a1_drag_origin?.initialRightEdge || 0);
            const deltaCols = Math.round(delta / spacingX);
            const newCols = Math.max(1, (a1_drag_origin?.initialCols || currentCols) + deltaCols);

            if (newCols !== currentCols) {
              createOrUpdateDotGrid({
                ...meta,
                cols: newCols
              }, true);
            }
          ...
          case 'a1_box': {
            const dx = pointer.x - a1_drag_origin.x;

            // ✅ Add a throttle threshold — ignore movement below this
            const threshold = 4;
            if (Math.abs(dx) < threshold) break;

            // ✅ Compute new width and clamp it
            let newWidth = a1_drag_origin.initialWidth + (handleName === 'a1_handle_r' ? dx : -dx);
            newWidth = Math.max(10, newWidth);

            // ✅ Adjust group left so that box expands symmetrically from the center
            let newLeft = a1_drag_origin.initialLeft + (handleName === 'a1_handle_r' ? dx : -dx) / 2;

            group.set({ width: newWidth, left: newLeft, scaleX: 1 });
            group.setCoords();

            const boxWidthInput = document.getElementById('a1_box_width');
            if (boxWidthInput) {
              boxWidthInput.value = Math.round(newWidth);
              boxWidthInput.dispatchEvent(new Event('input', { bubbles: false }));
            }
      ...
      // 👉 Left handle
      if (handleName === 'a1_handle_l') {
        switch (groupName) {
          case 'a1_dotgrid': {
            const spacingX = group.metadata?.spacingX || 20;
            const colsInput = document.getElementById('a1_edit_cols');
            const delta = a1_drag_origin.initialX - pointer.x;
            const deltaCols = Math.round(delta / spacingX);
            const newCols = Math.max(1, a1_drag_origin.initialCols + deltaCols);
            if (colsInput && parseInt(colsInput.value) !== newCols) {
              colsInput.value = newCols;
              colsInput.dispatchEvent(new Event('input', { bubbles: false }));
            }
            break;
          }

          case 'a1_lineset': {
            const originalRightEdge = a1_drag_origin.groupLeft + a1_drag_origin.groupWidth;
            const newLeft = Math.min(pointer.x, originalRightEdge - 10); // Prevent negative width
            const newWidth = Math.max(originalRightEdge - newLeft, 10);

            createOrUpdateLineset({
              ...group.metadata,
              width: newWidth
            }, true, 'left', newLeft); // 👈 pass newLeft

            // After recreating the group, adjust its left to newLeft
            const activeGroup = window._a1_active_group;
            activeGroup.set({ left: newLeft });
          ...
          // case 'a1_lineset': {
          //   const dx = a1_drag_origin.x - pointer.x;
          ...
          // case 'a1_box': {
          //   const dx = a1_drag_origin.pointerX - pointer.x;
          //   let newWidth = a1_drag_origin.initialWidth + dx;
          //   newWidth = Math.max(10, newWidth);
          //   const newLeft = a1_drag_origin.initialLeft - dx / 2;
      ...
      // 👉 Top/Bottom handle
      if ((handleName === 'a1_handle_t' || handleName === 'a1_handle_b') && a1_vertical_drag_origin) {
        handle.set({ top: pointer.y });
        group.setCoords();
        canvas.renderAll();

        const direction = a1_vertical_drag_origin.direction;

        switch (groupName) {
          case 'a1_lineset': {
            const spacing = parseFloat(document.getElementById('a1_edit_spacing')?.value || '20');
            const deltaY = pointer.y - a1_vertical_drag_origin.y;

            let change = Math.round(deltaY / spacing);
            if (direction === 'top') change = -change; // drag upward = add lines upward

            const newLineCount = Math.max(1, a1_vertical_drag_origin.lineCount + change);

            // Create new lineset with updated numLines and direction
            const newMeta = {
              ...group.metadata,
            ...
            createOrUpdateLineset({
              ...group.metadata,
              numLines: newLineCount
            }, true, direction, null, a1_vertical_drag_origin.top);
          ...
          case 'a1_dotgrid': {
            ...
            const spacingY = meta?.spacingY || 20;
            const currentRows = meta?.rows || 5;

            // Use top edge as fixed anchor, not bottom edge
            const delta = pointer.y - (a1_vertical_drag_origin?.initialTopEdge || 0);
            const newRows = Math.max(1, Math.round(delta / spacingY));
          ...
          //   // const delta = pointer.y - (a1_drag_origin?.initialBottomEdge || 0);
          //   // const deltaRows = Math.round(delta / spacingY);
          //   // const newRows = Math.max(1, (a1_drag_origin?.initialRows || currentRows) + deltaRows);

          //   const delta = pointer.y - (a1_drag_origin?.initialTop || 0);
          //   const newRows = Math.max(1, Math.round(delta / spacingY));
          ...
          case 'a1_box': {
            const dy = pointer.y - a1_vertical_drag_origin.y;
            const threshold = 4;
            if (Math.abs(dy) < threshold) break;

            let newHeight;
            let newTop;

            if (direction === 'top') {
              newHeight = a1_vertical_drag_origin.initialHeight - dy;
              newTop = a1_vertical_drag_origin.initialTop - dy / 2;
            } else {
              newHeight = a1_vertical_drag_origin.initialHeight + dy;
              newTop = a1_vertical_drag_origin.initialTop + dy / 2;
            }
            ...
            group.set({ height: newHeight, top: newTop, scaleY: 1 });
            group.setCoords();

            const boxHeightInput = document.getElementById('a1_box_height');
            if (boxHeightInput) {
              boxHeightInput.value = Math.round(newHeight);
              boxHeightInput.dispatchEvent(new Event('input', { bubbles: false }));
            }
          ...
          // case 'a1_box': {
          //   const dy = pointer.y - a1_vertical_drag_origin.pointerY;
          //   let newHeight;
          //   if (direction === 'top') {
          //     newHeight = a1_vertical_drag_origin.initialHeight - dy;
          //     const newTop = a1_vertical_drag_origin.initialTop - dy / 2;
          //     group.set({ height: newHeight, top: newTop, scaleY: 1 });
          //   } else {
          //     newHeight = a1_vertical_drag_origin.initialHeight + dy;
          //     const newTop = a1_vertical_drag_origin.initialTop + dy / 2;
          ...
          //   const boxHeightInput = document.getElementById('a1_box_height');
          //   if (boxHeightInput) {
          //     boxHeightInput.value = Math.round(Math.max(newHeight, 10));
          //     boxHeightInput.dispatchEvent(new Event('input', { bubbles: false }));
          //   }
          //   break;
          // }
    ...
    /* uncommenting new code for dotgrid handles
    
        function attachHandlesToDotGrid(group) {
          const canvas = lumise.stage().canvas;
    
          removeA1DotGridHandles(); // Clean up previous
    
          const handleSize = 8;
          const offset = 30;
    
          const handleData = [
            { name: 'top', x: 0, y: -offset, prop: 'rows', delta: -1 },
            { name: 'bottom', x: 0, y: group.height + offset, prop: 'rows', delta: 1 },
            { name: 'left', x: -offset, y: group.height / 2, prop: 'cols', delta: -1 },
            { name: 'right', x: group.width + offset, y: group.height / 2, prop: 'cols', delta: 1 }
          ];
    
          handleData.forEach(handle => {
            const dot = new fabric.Circle({
              radius: handleSize,
              fill: 'blue',
              originX: 'center',
              originY: 'center',
              left: group.left + handle.x,
              top: group.top + handle.y,
              hasControls: false,
              hasBorders: false,
              selectable: true,
              hoverCursor: 'pointer'
            ...
            dot.handleFor = 'a1_dotgrid';
    
            dot.on('mousedown', function () {
              dot.__origX = dot.left;
              dot.__origY = dot.top;
            });
    
            dot.on('moving', function (e) {
              const dx = dot.left - dot.__origX;
              const dy = dot.top - dot.__origY;
    
              const threshold = 20;
    
              if (handle.prop === 'rows' && Math.abs(dy) >= threshold) {
                const newMeta = { ...group.metadata };
                newMeta.rows = Math.max(1, newMeta.rows + handle.delta);
                createOrUpdateDotGrid(newMeta, true);
              } else if (handle.prop === 'cols' && Math.abs(dx) >= threshold) {
                const newMeta = { ...group.metadata };
                newMeta.cols = Math.max(1, newMeta.cols + handle.delta);
                createOrUpdateDotGrid(newMeta, true);
              }
            });
        ...
        function removeA1DotGridHandles() {
          const canvas = lumise.stage().canvas;
          const handles = canvas.getObjects().filter(obj => obj.handleFor);
    ...
    //Show Dimensions
    // --- Inside your initA1Designer(canvas) function ---

    function a1_showDimensions(targetGroup) {
      const objectToDisplayDimensions = targetGroup;
      const canvas = lumise.stage().canvas; // Get canvas reference here

      // ... (rest of your existing console.logs) ...

      if (objectToDisplayDimensions && (
        objectToDisplayDimensions.object_type === 'a1_lineset' ||
        objectToDisplayDimensions.type === 'a1_lineset' ||
        objectToDisplayDimensions.object_type === 'a1_dotgrid' ||
        objectToDisplayDimensions.type === 'a1_dotgrid' ||
        objectToDisplayDimensions.object_type === 'a1_box' ||
        objectToDisplayDimensions.type === 'a1_box' ||
        objectToDisplayDimensions.object_type === 'a1_checkboxes' ||
        objectToDisplayDimensions.type === 'a1_checkboxes'
      )) {
        if (lumise.ops.window_width >= 768) { // Check for desktop view
          const size = lumise.get.obj_size(objectToDisplayDimensions);
          const sizeText = size.map(value => `${value} (cm)`).join(" x ");

          const bbox = objectToDisplayDimensions.getBoundingRect(true); // Absolute bounding box
          const invertColor = lumise.get.color("invert");
          const textColor = (invertColor === '#333' ? '#777' : '#ccc');

          const paddingTop = 10; // Adjust this value for desired spacing above the object

          // --- CREATE OR UPDATE THE FABRIC.TEXT OBJECT ---
          if (!_a1_dimension_text_object) {
            // Create the text object if it doesn't exist
            _a1_dimension_text_object = new fabric.Text(sizeText, {
              left: 0, // Initial dummy values, will be calculated and set below
              top: 0,
              fontFamily: 'Roboto', // Ensure 'Roboto' font is loaded or pick a common one like 'Arial'
              fontSize: 12,
              fill: textColor,
              selectable: false,    // Make it non-selectable
              evented: false,       // Make it not respond to mouse events
              objectCaching: false, // Recommended for dynamic text to prevent rendering artifacts
              visible: true,        // Ensure it's visible initially
              // --- KEY CHANGE: Set origin points for easier positioning ---
              originX: 'center',    // This means the 'left' coordinate will be the horizontal center of the text
              originY: 'bottom'     // This means the 'top' coordinate will be the bottom edge of the text
            });
            canvas.add(_a1_dimension_text_object); // Add it to the canvas
          } else {
            // If it already exists, just update its text content and color first
            // This gives Fabric.js a chance to calculate its new width/height if text changed
            _a1_dimension_text_object.set({
              text: sizeText,
              fill: textColor,
              visible: true // Ensure it remains visible
            });
          }

          // --- CALCULATE NEW POSITION BASED ON OBJECT'S BOUNDING BOX AND TEXT'S ORIGINS ---
          // newTextX: The horizontal center of the object
          const newTextX = bbox.left + (bbox.width / 2);
          // newTextY: The top edge of the object minus the padding (since text's originY is 'bottom')
          const newTextY = bbox.top - paddingTop;
          ...
          // Always bring the text object to the front and update its internal coordinates
          _a1_dimension_text_object.bringToFront();
          _a1_dimension_text_object.setCoords();

        } else {
          // If window is too small, or other conditions apply, hide the text
          if (_a1_dimension_text_object) {
            console.log('  Hiding _a1_dimension_text_object (window width)');
            _a1_dimension_text_object.set({ visible: false });
          }
        }
      } else {
        // If no custom object is currently active, hide the dimension text object
        if (_a1_dimension_text_object) {
          console.log('  Hiding _a1_dimension_text_object (no custom object active)');
          _a1_dimension_text_object.set({ visible: false });
        }
      }
    }
    ...
    //       const elm_displaySize = document.getElementById('elm_displaySize');
    //       elm_displaySize.textContent = sizeText;
    //       console.log(elm_displaySize)

    //       // No explicit canvas.renderAll() needed here, as the after:render event ensures redrawing.
    //     }
    ...
    // --- Inside your initA1Designer(canvas) function ---
    // --- This replaces any previous canvas.on('after:render', ...) code you might have ---

    // 1. On Object Selection: Show/Update dimensions for custom objects
    canvas.on('object:selected', function (e) {
      const selectedObject = e.target;
      if (selectedObject && selectedObject.object_type && selectedObject.object_type.startsWith('a1_')) {
        // window._a1_active_group = selectedObject; // Ensure our global reference is set
        a1_showDimensions(selectedObject); // Call to show/update dimensions
        // canvas.renderAll(); // Ensure canvas updates to draw text
      }
    });


    // --- End of initA1Designer(canvas) function ---
  ...
  } //end of initA1Designer


  if (document.querySelector('li[data-tool="custom-save-load"]') === null) {
    const navLeft = document.querySelector('ul[data-block="left"][data-resp="file"]');
    const childrenNavLeft = navLeft.children;

    // 1. Find and store a reference to the 'Print' <li> element
    let printLiElement = null;
    for (let i = 0; i < childrenNavLeft.length; i++) {
      if (childrenNavLeft[i].getAttribute('data-tool') === 'print') {
        printLiElement = childrenNavLeft[i];
        break; // Found it, no need to continue looping
      }
    }

    // 2. Hide all existing children (as per your original code)
    for (let i = 0; i < childrenNavLeft.length; i++) {
      childrenNavLeft[i].style.display = 'none';
    }

    if (navLeft) {
      console.log('✅ Injecting custom nav tools');

      const nameLi = document.createElement('li');
      const nameInput = document.createElement('input');
      nameInput.id = "a1-design-name";
      nameInput.placeholder = "Name Your Design";
      nameInput.style.width = "200px";
      nameInput.style.border = "none";
      nameInput.style.borderRadius = "3px";
      nameInput.style.padding = "5px 5px";
      nameInput.style.borderBottom = "2px solid green";
      nameLi.appendChild(nameInput);
      navLeft.appendChild(nameLi);

      const saveLi = document.createElement('li');
      const saveSpan = document.createElement('span');
      saveSpan.id = 'a1customSaveBtn';
      saveSpan.style.width = '100%';
      saveSpan.style.cursor = 'pointer';
      saveSpan.textContent = '💾 Save';
      saveLi.appendChild(saveSpan);
      navLeft.appendChild(saveLi);

      const saveAsLi = document.createElement('li');
      saveAsLi.style.display = 'none';
      const saveAsSpan = document.createElement('span');
      saveAsSpan.id = 'a1customSaveAsBtn';
      saveAsSpan.style.width = '100%';
      saveAsSpan.style.cursor = 'pointer';
      saveAsSpan.textContent = '💾 Save As New Design';
      saveAsLi.appendChild(saveAsSpan);
      navLeft.appendChild(saveAsLi);

      const loadLi = document.createElement('li');
      const loadSpan = document.createElement('span');
      loadSpan.id = 'a1customLoadBtn';
      loadSpan.style.width = '100%';
      loadSpan.style.cursor = 'pointer';
      loadSpan.textContent = '📂 Load';
      loadLi.appendChild(loadSpan);
      navLeft.appendChild(loadLi);

      // 3. Append the 'Print' <li> element after the Load button and make it visible
      if (printLiElement) {
        navLeft.appendChild(printLiElement);
        printLiElement.style.display = ''; // Overwrite the 'display: none'
      }
    }
  ...
  // --- Place this code within the main jQuery(document).ready(function ($) { ... }); block ---
  // --- It should run after your custom save/load buttons are added and the 'printLiElement' is made visible ---

  // Function to inject the Download PDF button and attach its event listener
  function injectDownloadPdfButton() {
    const printLiElement = document.querySelector('li[data-tool="print"]');

    if (printLiElement) {
      const printNavUl = printLiElement.querySelector('ul#lumise-print-nav');
      if (printNavUl) {
        // Locate the <li> that contains the existing "Print Now" and "Download" buttons
        const buttonsContainerLi = printNavUl.querySelector('li:last-child'); // This targets the last <li> which holds the buttons

        if (buttonsContainerLi && !buttonsContainerLi.querySelector('#a1DownloadPdfButton')) {
          // Create the new button HTML
          const downloadPdfButtonHtml = `
                    <button class="lumise-btn blue" id="a1DownloadPdfButton" style="margin-top: 10px;">
                        Download PDF
                        <i class="lumisex-file-pdf"></i>
                    </button>
                `;
          // Insert the new button HTML at the end of the existing buttons <li>
          buttonsContainerLi.insertAdjacentHTML('beforeend', downloadPdfButtonHtml);

          // Attach the click event listener to the new button
          document.getElementById('a1DownloadPdfButton')?.addEventListener('click', function (e) {
            e.preventDefault();

            // Get the current settings from the print navigation panel
            // These parameters are typically used for PDF export in Lumise's internal functions
            const include_base = document.getElementById('lumise-print-base')?.checked || false;
            const full_pages = document.getElementById('lumise-print-full')?.checked || false;
            const cropmarks = document.getElementById('lumise-print-cropmarks')?.checked || false;

            // Trigger the PDF download functionality
            // lumise.fn.download_design is the core function for file downloads
            lumise.fn.download_design({
              type: 'pdf', // Specify PDF format
              include_base: include_base,
              full: full_pages, // 'Export all pages?' setting
              cropmarks: cropmarks // 'Crop marks & bleed?' setting
            });
          });
        }
      }
  ...
  // You might need to adjust when this function is called if the 'Print' <li>
  // isn't immediately available after your current script runs.
  // A safe place is often after Lumise's main initialization is complete,
  // or directly after your previous code that re-appends printLiElement.

  // Example of where to call it, assuming your previous code runs first:
  // (You would add this after the section where you re-append printLiElement in your app.min.js)
  // For instance, right after: `if (printLiElement) { navLeft.appendChild(printLiElement); printLiElement.style.display = ''; }`
  // you could add:
  // setTimeout(injectDownloadPdfButton, 500); // A small delay might be necessary to ensure DOM is fully ready
  injectDownloadPdfButton(); // Or call it directly if elements are guaranteed to be present.


  function cleanFontNames(json) {
    if (json.objects && Array.isArray(json.objects)) {
      json.objects.forEach(obj => {
        if (obj.fontFamily && typeof obj.fontFamily === 'string') {
          // Remove accidental double-quotes around the font name
          obj.fontFamily = obj.fontFamily.replace(/^"+|"+$/g, '');
        }
        if (obj.objects) {
          // Handle grouped objects recursively
          cleanFontNames(obj);
        }
      });
    }
  ...
  document.addEventListener('click', function (e) {
    if (e.target && e.target.id === 'customSaveBtn') {
      const canvas = lumise.stage().canvas;

      const rawJSON = canvas.toJSON(['object_type', 'custom_type', 'spacing', 'name']);
      const cleanedJSON = cleanFontNames(rawJSON);
      ...
      xhr.send('action=save_custom_lumise_design&design_json=' + encodeURIComponent(JSON.stringify(wrappedJSON)));
    }
  });

  // Save button - multiple designs
  document.getElementById('a1customSaveBtn').addEventListener('click', function () {

    const name = document.getElementById('a1-design-name').value.trim();
    if (!name) return alert('❌ Please enter a design name');

    const canvas = lumise.stage().canvas;

    const rawJSON = canvas.toJSON(['object_type', 'custom_type', 'spacing', 'name', 'metadata']);
    const cleanedJSON = cleanFontNames(rawJSON);
    ...
    // const designData = JSON.stringify(lumise.get.product());

    const designData = JSON.stringify(wrappedJSON);


    const payload = new URLSearchParams();

    const isUpdate = !!window._a1_loaded_design_id;

    payload.append('action', isUpdate ? 'update_a1_custom_design' : 'save_a1_custom_design');
    payload.append('name', name);
    payload.append('data', designData);

    if (isUpdate) {
      payload.append('id', window._a1_loaded_design_id);
    }
    ...
    fetch(ajaxurl, {
      ...
      .then(json => {
        if (json.success) {
          alert(isUpdate ? '✅ Design updated' : '✅ Design saved');
        } else {
          alert('❌ Save failed');
        }
      });
  ...
  document.getElementById('a1customSaveAsBtn').addEventListener('click', function () {
    const inputEl = document.getElementById('a1-design-name');
    const originalName = inputEl.value.trim();

    if (!originalName) {
      return alert('❌ Cannot duplicate unnamed design');
    }

    const newName = originalName + ' - Copy';
    inputEl.value = newName; // optional: update UI to reflect it
    ...
    const payload = new URLSearchParams();
    payload.append('action', 'save_a1_custom_design');
    ...
    fetch(ajaxurl, {
      ...
      .then(json => {
        if (json.success) {
          ...
          window._a1_loaded_design_id = null; // clear the old ID so future save becomes "new"
        } else {
  ...
  const productBaseURL = 'http://localhost/wordpress/wp-content/uploads/lumise_data/products/m9wp58fk.jpg';

  function stripProductBaseFromObjects(json) {
    if (json?.stages?.lumise?.data?.objects) {
      json.stages.lumise.data.objects = json.stages.lumise.data.objects.filter(obj => {
        return !(obj.type === 'image' && obj.src === productBaseURL);
      });
    }
    return json;
  }
  ...
  //load
  document.addEventListener('click', function (e) {
    if (e.target && e.target.id === 'customLoadBtn') {
      const xhr = new XMLHttpRequest();
      xhr.open('POST', ajaxurl, true);
      xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
      ...
      xhr.onload = function () {
        try {
          ...
          if (res.success && res.data) {
            const designData = JSON.parse(res.data);
            const cleanData = stripProductBaseFromObjects(designData);

            const fontsToLoad = ['Pacifico']; // Optional: Extract from designData
            ...
            const waitForFonts = () =>
              new Promise((resolve) => {
                WebFont.load({
                  ...
                  inactive: () => {
                    console.warn('⚠️ Fonts failed to load — continuing');
                    resolve();
                  }
                });
              });

            const waitForLumiseCanvas = () =>
              new Promise((resolve) => {
                const check = () => {
                  if (lumise.stage && lumise.stage().canvas) {
                    console.log('🎯 Lumise is ready — now importing');
                    resolve();
                  } else {
            ...
            Promise.all([waitForFonts(), waitForLumiseCanvas()]).then(() => {
              // Fix custom group types before import
              const fixCustomTypes = (data) => {
                ...
                objs.forEach((obj) => {
                  if (obj.object_type === 'a1_lineset' && obj.type === 'group') {
                    ...
                    obj.type = 'a1_lineset'; // ✅ critical fix
                  }
                  if (obj.object_type === 'a1_dotgrid' && obj.type === 'group') {
                    console.log('🔧 Fixing group to type a1_dotgrid before import');
                    obj.type = 'a1_dotgrid'; // ✅ critical fix
                  }
                  if (obj.object_type === 'a1_box' && obj.type === 'rect') {
                    console.log('🔧 Fixing group to type a1_dotgrid before import');
                    obj.type = 'a1_box'; // ✅ critical fix
                    if (obj.clipTo) {
                      console.log('🧹 Removing unsupported clipTo from a1_box before import');
                      delete obj.clipTo;
                    }
                  }
                  if (obj.object_type === 'a1_checkboxes' && obj.type === 'group') {
                    console.log('🔧 Fixing group to type a1_checkboxes before import');
                    obj.type = 'a1_checkboxes'; // ✅ critical fix
                  }
                });
              };

              fixCustomTypes(cleanData); // before lumise.tools.imports()

              lumise.tools.imports(cleanData, () => {
                alert('✅ Design loaded successfully');

                const group = lumise.stage().canvas
                  .getObjects()
                  .find((o) => o.type === 'a1_lineset');

                if (group) {
                  console.log('🎯 Group has', group._objects.length, 'lines after import');
                } else {
                  console.warn('⚠️ a1_lineset not found after import');
                }
              });
            });
          } else {
            alert('❌ No design found to load.');
          }
        } catch (ex) {
          console.error('❌ Load error:', ex);
          alert('❌ Error loading design: ' + ex.message);
        }
      };

      xhr.send('action=load_custom_lumise_design');
    }
  });

  // Load button for multiple designs
  document.getElementById('a1customLoadBtn').addEventListener('click', function () {
    const modal = document.createElement('div');
    modal.style.cssText = 'position:fixed;top:20%;left:30%;width:40%;background:white;border:1px solid #ccc;padding:20px;z-index:100002;max-height:400px;overflow-y:auto';
    modal.innerHTML = '<h3>📂 Select a design to load:</h3><div id="a1-design-list">Loading...</div><button id="a1-close-modal">Close</button>';
    document.body.appendChild(modal);

    document.getElementById('a1-close-modal').onclick = () => modal.remove();

    fetch(ajaxurl + '?action=list_a1_custom_designs')
      .then(res => res.json())
      .then(json => {
        console.log(json);
        const listEl = document.getElementById('a1-design-list');
        listEl.innerHTML = '';
        if (!json.success || !json.data.length) {
          listEl.textContent = 'No saved designs found.';
          return;
        }
        ...
        listEl.appendChild(tbContainer);

        const tbHeader = document.createElement('div');
        tbHeader.classList.add("table-row", "table-header");
        tbContainer.appendChild(tbHeader);

        // Add header items
        const headerItems = ["Name", "Date", "Actions"];
        headerItems.forEach(headerText => {
          const th = document.createElement('div');
          th.className = "table-cell";
          th.textContent = headerText;
          tbHeader.appendChild(th);
        });

        json.data.forEach(item => {
          const tbRow = document.createElement('div');
          tbRow.className = "table-row";

          const nameCell = document.createElement('div');
          nameCell.className = "table-cell";
          nameCell.innerHTML = `<strong>${item.name}</strong>`;
          tbRow.appendChild(nameCell);

          const dateCell = document.createElement('div');
          dateCell.className = "table-cell";
          dateCell.style.color = "#777";
          dateCell.textContent = item.date;
          tbRow.appendChild(dateCell);

          const actionsCell = document.createElement('div');
          actionsCell.className = "table-cell";
          const openButton = document.createElement('button');
          openButton.dataset.id = item.id;
          openButton.dataset.name = encodeURIComponent(item.name);
          openButton.className = "a1-load-single";
          openButton.textContent = "Open";
          actionsCell.appendChild(openButton);
          tbRow.appendChild(actionsCell);

          tbContainer.appendChild(tbRow);
        });

        listEl.querySelectorAll('.a1-load-single').forEach(btn => {
          btn.addEventListener('click', function () {
            const id = this.dataset.id;
            fetch(ajaxurl + '?action=load_a1_custom_design&id=' + id)
              .then(res => res.json())
              .then(json => {
                if (json.success && json.data) {
                  const designData = JSON.parse(json.data);
                  const cleanData = stripProductBaseFromObjects(designData);

                  // Fix any group-based custom types before import
                  ...
                  const fixCustomTypes = (data) => {
                    ...
                    objs.forEach(obj => {
                      ...
                      if (obj.object_type === 'a1_box') {
                        console.log('🔧 Fixing group to type a1_box before import');
                        obj.type = 'a1_box';
                      }
                    });
                  };
                  ...
                  fixCustomTypes(cleanData);

                  const fontsToLoad = ['Pacifico']; // Optional: you can extract from objects[] dynamically
                  ...
                  Promise.all([waitForFonts(), waitForLumiseCanvas()]).then(() => {
                    lumise.tools.imports(cleanData, () => {
                      ...
                      window._a1_loaded_design_id = id; // ✅ Store loaded design ID
                      document.getElementById('a1-design-name').value = decodeURIComponent(this.dataset.name);

                      //show save as new button
                      const saveAsLi = document.getElementById('a1customSaveAsBtn')?.parentElement;
                      if (saveAsLi) saveAsLi.style.display = 'block';
                      ...
                      const canvas = lumise.stage().canvas;
                      const dotGrids = canvas.getObjects().filter(o => o.object_type === 'a1_dotgrid');
                      console.log(`✅ Found ${dotGrids.length} a1_dotgrid objects on canvas`);

                      const objects = canvas.getObjects();

                      objects.forEach(obj => {
                        if (obj.object_type === 'a1_box' && obj.metadata) {
                          console.log('🎨 Restyling a1_box from metadata', obj.metadata);
                          obj.set({
                            stroke: obj.metadata.color || 'black',
                            strokeWidth: obj.metadata.thickness || 1
                          });
                          obj.setCoords(); // required if stroke width changes
                        }
                      });
